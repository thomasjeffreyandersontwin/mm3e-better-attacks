...
function parseMultipleEffectsFromNotes(power) {
  ...
  if (!notes.includes('<h3>')) return [];
  
  const effects = [];
  
  // Split by H3 tags to get each effect section
  const sections = notes.split(/<h3>/i);
  
  for (let i = 1; i < sections.length; i++) { // Skip first empty section
    const section = sections[i];
    
    // Extract effect name from H3 content
    const h3Match = section.match(/^([^<]+)</);
    if (!h3Match) continue;
    
    const fullEffectName = h3Match[1].trim();
    // Extract just the effect type (e.g., "Affliction" from "Affliction: Affliction 20")
    ...
    if (paragraphs.length >= 2) {
      // Skip first paragraph (descriptive), use second paragraph for data
      const effectData = paragraphs[1];
      
      effects.push({
        name: effectType,
        fullName: fullEffectName,
        data: effectData
      });
    }
  }
...
async function createLinkedAttackFromMultipleEffects(effects, actor, originalPower) {
  ...
  // Create primary attack from first effect
  ...
  const virtualPower = {
    ...
    system: {
      ...
      notes: linkedEffectNote // Use full original power notes, not just effect data
    }
  };
  ...
  // If there are additional effects, link them using the existing linked attack logic
  if (effects.length > 1) {
    for (let i = 1; i < effects.length; i++) {
      //we need to extract only the relevant part of the note eg after effect name and before next effect name
      ...
      const linkedVirtualPower = {
        ...originalPower,
        name: linkedEffect.name, // Use just the effect name, not the full H3 title
        system: {
          ...originalPower.system,
